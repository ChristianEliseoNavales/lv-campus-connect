...
router.get('/pie-chart/combined', verifyToken, checkApiAccess, analyticsController.getCombinedPieChart);

// GET /api/analytics/pie-chart/:department - Get service distribution for pie chart
router.get('/pie-chart/:department', verifyToken, checkApiAccess, validateDepartment, analyticsController.getPieChartByDepartment);

// GET /api/analytics/area-chart/:department - Get time series data for area chart
router.get('/area-chart/:department', verifyToken, checkApiAccess, validateDepartment, validateTimeRange, analyticsController.getAreaChartByDepartment);

// GET /api/analytics/dashboard-stats/:department - Get dashboard statistics
router.get('/dashboard-stats/:department', verifyToken, checkApiAccess, validateDepartment, analyticsController.getDashboardStats);
...
router.get('/combined/:department', verifyToken, checkApiAccess, validateDepartment, validateTimeRange, analyticsController.getCombinedAnalytics);

// GET /api/analytics/active-sessions - Get currently active users
router.get('/active-sessions', verifyToken, checkApiAccess, analyticsController.getActiveSessions);

// GET /api/analytics/queue-ratings-summary - Get queue ratings statistics
router.get('/queue-ratings-summary', verifyToken, checkApiAccess, analyticsController.getQueueRatingsSummary);

// GET /api/analytics/queue-by-department - Get queue distribution by department
router.get('/queue-by-department', verifyToken, checkApiAccess, analyticsController.getQueueByDepartment);

// GET /api/analytics/analytical-report/:role - Get comprehensive analytical report data
router.get('/analytical-report/:role', verifyToken, checkApiAccess, analyticsController.getAnalyticalReport);
