...
const Settings = () => {
  ...
  const fetchQueueSettings = async () => {
    try {
      const response = await fetch('http://localhost:5000/api/settings/queue/admissions');
      if (response.ok) {
  ...
  const fetchServices = async () => {
    try {
      const response = await fetch('http://localhost:5000/api/services/admissions');
      if (response.ok) {
        const data = await response.json();
        setServices(data);
      }
  ...
  const fetchAdminUsers = async () => {
    try {
      ...
      const response = await fetch('http://localhost:5000/api/users/by-role/admissions_admin');
      if (response.ok) {
        const data = await response.json();
        setAdminUsers(data);
      } else {
  ...
  const handleToggleQueueing = async () => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:5000/api/settings/queue/admissions/toggle', {
        method: 'PUT',
        headers: {
