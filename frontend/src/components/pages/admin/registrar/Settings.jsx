          ...
          <div className="p-5 space-y-5">
            ...
            <div>
              <label className="block text-sm font-semibold text-gray-900 mb-1.5">
                ...
                {windowFormData.isPriority && (
                  <span className="ml-1.5 text-xs text-gray-500">(Services from other windows auto-assigned)</span>
                )}
              </label>
              <div className={`border rounded-lg p-2.5 max-h-32 overflow-y-auto ${
                ...
                {windowFormData.isPriority ? (
                  <div className="text-center py-3">
                    ...
                    <p className="text-[10px] text-gray-500 mt-1">Priority windows automatically handle services assigned to other windows</p>
                  </div>
                ) : services.length === 0 ? (
...
const Settings = () => {
  ...
  useEffect(() => {
    ...
    console.log('🔌 Registrar Settings: Joining admin-registrar room');
  ...
  const fetchAdminUsers = async () => {
    try {
      ...
      // Fetch users with Registrar office (both Admin and Admin Staff)
      const response = await authFetch(`${API_CONFIG.getAdminUrl()}/api/users?office=Registrar&isActive=true&role=Registrar Admin,Registrar Admin Staff`);
      if (response.ok) {
  ...
  const fetchLocationSettings = async () => {
    try {
      const response = await authFetch(`${API_CONFIG.getAdminUrl()}/api/settings/location/registrar`);
      if (response.ok) {
        const data = await response.json();
  ...
  const handleToggleQueueing = async () => {
    ...
    try {
      ...
      const response = await authFetch(`${API_CONFIG.getAdminUrl()}/api/settings/queue/registrar/toggle`, {
        method: 'PUT',
        headers: {
          ...
          <div className="col-span-1 row-span-1 bg-white rounded-xl sm:rounded-2xl border border-gray-300 shadow-md p-3 sm:p-4 md:p-5">
            <div className="flex items-center justify-between">
              <div className="flex flex-col space-y-1 sm:space-y-1.5">
                ...
                <div className="h-2.5 sm:h-3 bg-gray-200 rounded w-20 sm:w-24 md:w-28 animate-pulse"></div>
              </div>
                  ...
                  <div key={index} className="p-2 sm:p-2.5 bg-gray-50 rounded-lg border border-gray-200 animate-pulse">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-2 sm:space-x-2.5">
                        <div className="h-2.5 sm:h-3 bg-gray-200 rounded w-20 sm:w-24 md:w-28"></div>
          ...
          <div className="col-start-2 row-start-2 row-span-3 bg-white rounded-xl sm:rounded-2xl border border-gray-300 shadow-md p-3 sm:p-4 md:p-5">
            <div className="mb-2 sm:mb-2.5 md:mb-3">
              <div className="grid grid-cols-3 gap-2 sm:gap-3 items-center p-2 sm:p-2.5">
                ...
                <div className="h-4 sm:h-4.5 md:h-5 bg-gray-200 rounded w-20 sm:w-24 md:w-28 animate-pulse"></div>
              ...
              {[...Array(4)].map((_, index) => (
                <div key={index} className="grid grid-cols-3 gap-2 sm:gap-3 p-2.5 sm:p-3 bg-gray-50 rounded-lg border border-gray-200 animate-pulse">
                  ...
                  <div className="flex flex-col justify-center space-y-0.5 sm:space-y-1">
                    ...
                    <div className="h-2.5 sm:h-3 bg-gray-200 rounded w-24 sm:w-28 md:w-32"></div>
                  </div>
